...
do sinal a ser detetado de forma que se realcem as propriedades necessárias a
...
inerente as variáveis no espaço compactado provido pelo algoritmo
T2Calo. Desenvolvemos então um detetor neural que atenda os critérios da
filtragem de elétrons, combinado a este algoritmo de extração de
características. Este novo sistema de discriminação apresenta uma eficiência
...
neural. Neste momento conduzimos uma análise de relevância das variáveis que
...
de entrada do detetor neural, aumentando desta forma, o nível de compactação
...
conduzida tendo em vistas seu poder de discriminação. Finalmente, um detetor é
desenvolvido baseando-se em apenas 14 dos 100 anéis iniciais é definido,
baseando-se na relevância discriminativa dos anéis de deposição energética,
atingindo $97,59$\% na deteção de elétrons contra um falso alarme de
$3,22$\%.
...
entre o CERN e UFRJ. As contribuições deste trabalho também incluem o
desenvolvimento e construção de boa parte do Segundo Nível de Filtragem e sua
...
disponíveis no LVL2. O algoritmo de deteção de elétrons nestes dois níveis de
...
O Capítulo~\ref{chap:neural} começa trazendo uma revisão das as técnicas de
análise multi-variável que são atualmente empregadas em experimentos em Física
...
que seja possível construir e executar sistemas de deteção neural dentro deste
