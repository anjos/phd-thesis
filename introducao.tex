...
orbitas dos planetas tinha os tamanhos relativos observados. Seu acesso a
observações acuradas realizadas por outro cientista da época o capacitou a
...
planetas não descrevem uma órbita circular, mas elípticas, tendo o Sol em um
de seus focos. Kepler também sugeriu que uma ``força'' emanado do Sol
afastaria os planetas de suas órbitas naturais, levando-os a perseguir uma
órbita elíptica.
...
anteceder os resultados teóricos, eliminaram a aceitação de ``dogmas'' e
fizeram nascer uma nova era onde as idéias científicas eram abertamente
discutidas e rigorosamente testadas.
...
hoje e encompassa as \idx{forças fundamentais} ``forte'', ``fraca'' e
...
modelo padrão ainda não foi observado até os dias de hoje.Alguns experimentos,
dentre os quais o experimento \idx{ATLAS} (do inglês,
\eng{A Toroidal LHC Apparatus}) no
...
eletricamente carregadas são posicionadas no interior de um túnel e excitadas
a acelerar por pulsos eletromagnéticos\index{pulsos eletromagnéticos}. A
...
eletricamente carregados adquirem força (aceleração) quando envoltos por um
...
desvantagem: quanto maior a energia necessária no centro de impacto, maior
...
par\-tí\-culas, elas são colocadas em rota de colisão com um alvo fixo ou,
então, com as partículas de outro feixe acelerado. Cada uma dessas colisões
...
decaírem em outras partículas, sem deixar pistas aparentemente detetáveis.
...
juntos para detetar um evento, partículas individuais podem ser distinguidas
da multidão para efeito de análise e o evento original pode ser reconstruído.
...
rota e curvatura de partículas, por isto é chamado de detetor de trajetória,
arrasto ou, mais comumumente, traços. A curvatura das partículas sobre o campo
magnético a que são expostas no detetor dá informações sobre a carga da
partícula e seu momento. O segundo detetor é chamado calorímetro
eletromagnético (e.m.) e tem a função de determinar a energia total de
elétrons, pósitrons e fótons (raios $\gamma$). Estas partículas decaem,
formando chuveiros de outros elétrons, pósitrons e fótons por dentro do
calorímetro e.m.. O terceiro detetor, o calorímetro hadrônico, mede a energia
total de chuveiros originados por hádrons (prótons, nêutrons ou mésons). É
composto de material pesado, que força o decaimento dos hádrons num chuveiro
hadrônico, bem mais largo e profundo que o eletromagnético. O quarto detetor é
um detetor de múons. Somente estas partículas e neutrinos escapam dos outros
sistemas de deteção. Neutrinos, infelizmente, nem por este último são
detetados. Sua massa pode ser calculada, no entanto, através da energia
faltante no evento. A Figura~\ref{fig:decay} mostra como alguns tipos de
...
massa da partícula será convertida em radiação ou ``calor'', o que explica o
nome deste tipo de detetor. É claro, nenhuma temperatura é medida, mas as
...
carregadas. A calorimetria é também a única forma previsível de medir
partículas neutras entre as secundárias produzidas em uma colisão de altas
energias \cite{bock:detector, booth}.
...
Quanto à construção, é possível distinguir-se os seguintes tipos de
calorímetro:
...
\begin{itemize}
  ...
  \item \textbf{Heterogêneos:}\index{Calorímetro!Heterogêneo} Também
  conhecidos como \idx{ca\-lo\-rí\-me\-tros de a\-mos\-tra\-gem}. Neste tipo
  de detetor as funções de absorção de energia e leitura do sinal são
...
calorimetria é maior para calorímetros que detetores de traços, e soluções
bastante engenhosas foram encontradas nos últimos 15 anos \cite{hadcal}.
...
\item {\bf radiação:} através do fenômeno conhecido como
  ...
  partículas geradas por este fenômeno, se tiverem energia, repetirão este
  ...
  número atômico, maiores as chances de perda por radiação \cite{hadcal}.
...
chuveiros hadrônicos tem componentes eletromagnéticas.

Quando um hádron altamente ener\-gé\-tico penetra um bloco de ma\-té\-ria,
...
[Falta uma figura de um calorímetro hadrônico, de preferência não-LHC.]

Sumarizando as razões pelas quais os calorímetros emergiram como
detetores-chave em praticamente todos os experimentos em física de partículas
encontramos:
...
experimentos com detetores e o sinal discutido ativa o sistema de gravação de
eventos para registrar parte ou a totalidade dos sinais captados em mídia
...
proibitivo economicamente, especialmente considerando o valor relativo dos
dados coletados. Os programas de reconstrução de eventos atuais também
consomem mais e mais poder de processamento e, se a entrada poder ser
sensivelmente reduzida em volume, menor a quantidade de recursos
computacionais serão necessários.

O enfoque usado para projetar e implementar um sistema de filtragem depend
fortemente no domínio do problema. Um experimento em Física de Altas Energias
típico deve responder a um novo evento a cada 10 nanossegundos
aproximadamente. O \idx{tamanho do evento} para grandes experimentos (veja a
Figura~\ref{fig:eventsize}) também tende a ser grande, chegando a alguns
...
Experimentos modernos usam um sistema de filtragem divido em múltiplos níveis
para endereçar o problema das taxas de entrada. Os níveis mais baixos
implementam algorítmos simples e rápidos, normalmente usando FPGA's e
\eng{hardware} personalizado, que removem \eng{backgrounds} mais simples e
