...
partículas, já que há uma grande probabilidade e espalhamento múltiplo (do
...
compacta a informação de entrada (O(100)-O(1000)), que normalmente possui
dimensionalidade bastante elevada, para um conjunto pequeno (O(10)) de
varíaveis que possam ser analisadas mais facilmente.
...
No caso de elétrons com valores energéticos até algumas dezenas de
...
energética entre a segunda camada da seção e.m. e a energia total do objeto
considerando-se uma área $0,4\times0,4$ no plano $\eta\times\phi$. Os eventos
considerados são simulações de elétrons e jatos cobrindo um campo energético
de algumas dezenas de GeV até 90 GeV e interagindo com os calorímetros do
ATLAS. Observa-se que para elétrons, a percentagem da energia total é
naturalmente maior que para jatos.

PEDIR AO THIAGO UMA PLOT E X JATO DA ENERGIA DEPOSITADA NA 2A. CAMADA/ETOTAL

\paragraph{Classificação -} A fase seguinte à compactação do espaço original
de entrada é a discriminação da Física de interesse. Tipicamente
\cite{hlt-tdr, zeus-neural} a deteção é realizada por meio de classificadores
bayesianos, por 
