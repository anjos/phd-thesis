...
\begin{itemize}
\item Elétrons simulados via Monte Carlo, com energia transversa fixa em 25
GeV. Os elétrons de cada evento simulado interagem com diferentes regiões do detetor;
\item Jatos simulados via Monte Carlo, com energia transversa variante, à
partir de 17 GeV;
\end{itemize}
...
na caverna do experimento. Um \eng{background} com a luminosidade inicial do
experimento ($10^{33}cm^{-2}s^{-1}$) também está adicionado aos dados,
provendo um ambiente bastante acurado dentro do que se é possível prever em
termos de operação e exequível em um ambiente de simulação. 
...
aceitando todos os eventos com apenas 1 objeto tipo EM com as seguintes
características: 

\begin{itemize}
\item ao menos 20 GeV numa região de $2\times2$ torres de filtragem na seção
e.m.;
...
PERGUNTAR AO THIAGO QUAIS SÃO AS CARACTERÍSTICAS ATUAIS DE FILTRAGEM DO LVL1
