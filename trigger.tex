...
eventos de interesse. A taxa de eventos produzida pelo LHC será de 40 milhões
por segundo, ou 40 MHz em bateladas que durarão cerca de 10 horas (este
período é comumente referido como \eng{run}). Cada evento poderá produzir uma
colisão favorável a Física de interesse embora a grande maioria, mais de
99,9999\%, represente Física já bastante estudada em experimentos anteriores
ainda que a luminosidade do sistema seja tão elevada. Ademais, cada evento
detetado pelo ATLAS produzirá cerca de 1,5 Megabytes em dados, o que torna o
problema da filtragem bastante difícil, já que o fluxo de dados que deve ser
analisado por base de tempo está acima da capacidade de qualquer tecnologia de
transporte de dados atual.

Para o experimento ATLAS, o sistema de filtragem será construído em 3 níveis
seqüenciais (veja a Figura~\ref{fig:trigger-sketch}). Cada nível que sucede
...
subconjunto dos detetores do ATLAS, os calorímetros e os detetores de múons
\cite{l1-tdr}. Múons com altos valores de \idx{momento transverso} ($p_T$) são
identificados a partir das câmaras de trigger conectadas ao Spectrômetro de
...
Os eventos selecionados pelo LVL1 são lidos a partir eletrônica da primária dos
...
do evento} (do inglês, \idxeng{Event Building}, EB). Porém, até a fase de
...
mapa de RoI's destacadas pelo LVL1 e comunica, através de uma conexão serial
...
guiando o operador e sinalizando comportamento anormal do maquinário. O DCS
...
número e o DCS guarda o estado operacional do detetor enquanto os dados estão
sendo adquiridos, correlacionando os números dos eventos com a qualidade da
...
constantemente. Por outro lado, o DAQ é somente necessário durante a aquisição
...
dados que podem ser adquiridos em um período de funcionamento do
...
capturam e servem informação ligados à monitoração dos diversos processos do
...
aplicações. Interfaces (gráficas) aos operadores permitem configurar e
...
de ``L1R''. A mensagem que chega ao L2SV representa portanto uma avaliação do
evento com qualidade equivalente à visibilidade que o LVL1 tem do
detetor. Para continuar com a avaliação do evento, o L2SV alocará uma L2PU que
estiver sob sua guarda para processá-lo.
...
também significam maior banda passante necessária a rede que interliga estes
...
durante o momento ligação das máquinas. A conexão concomitante de 2.500 discos
...
executam a análise da Física de interesse são carregados dinamicamente na L2PU
...
encontrados. Algumas modalidades de algoritmos podem requer mais dados e,
...
valores de tamanhos de RoI realísticos para dados partindo da seção e.m. do
...
com outros 5 ROB's cuja a numeração sucederia o primero ROB. Se o último ROB
