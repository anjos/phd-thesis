...
apenas, através de técnicas simples de filtragem codificadas em \eng{hardware}
...
aproximadamente 1 kHz utilizando aplicações codificadas em \eng{software} para
...
conectadas ao Espectrômetro de Múons (veja a Seção~\ref{sec:atlas-muon}) na
...
de cada detetor por meio de \eng{drivers} denominados \idxeng{ReadOut Drivers}
ou \idx{ROD}'s. Cerca de 1.600 ROD's serão necessários para ler todos os dados
...
chamados de \idxeng{derandomizers}, garantem um fluxo contínuo dos dados aos
...
como \idxeng{Timing, Trigger and Control distribution}, ou TTC), que gera,
...
indicando ao processador de construção de eventos (\eng{SubFarm Input}, SFI)
que deverá coletar os fragmentos do evento. O SFI então envia mensagens a cada
um dos ROS's no sistema e requer todas as informações disponíveis para o
evento de interesse. O objetivo do SFI é aglutinar todas as partes do evento
em um único espaço de memória, liberando as partes anteriores do sistema do
encargo do gerenciamento deste mesmo espaço.
...
análoga no LVL2, dada a taxa reduzida de eventos remanescentes. Se aprovado, o
...
processadores disponíveis em cada máquina. Em máquinas duplamente processadas,
...
(sistemas duplamente processados) é de, aproximadamente, três e é independente
...
processadoras, a coleção de dados contribui com menos de 10
...
é independente do tempo de execução observado.
