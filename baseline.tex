...
ou Extração de Características (do inglês, \eng{Feature Extraction} ou
...
Para trazer uma eficiência compatível com este ambiente, o Sistema de
...
objetos com alto valor de momento transverso são, entre outros, \eng{léptons}
...
\eng{LVL1 Result}) ou \gls{glos:l1r}, como visto na
...
(marcado na figura como \eng{Read Out}) que carregue todos os dados do detetor
em seus
\eng{buffers}.
...
\ding{187}). Este sumário contém valores refinados de energia, momento e
localização dos objetos explorados no contexto LVL2. Analogamente ao resultado
do LVL1, o resultado do LVL2 é chamado \gls{glos:l2r} (do inglês \eng{LVL2
Result}). Espera-se que o LVL2 atinja uma redução da taxa de eventos de
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.98]{em-had-percent}
\end{center}
...
Uma das estratégias em consideração atualmente \cite{t2calo-wiki}, pondera
...
\eng{Receiver Operating Characteristics}, \gls{glos:roc} \cite{vantrees}), contém os
