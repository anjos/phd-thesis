...
as áreas neste campo, desde da concepção de detetores \cite{wilk-nim-2006},
...
energia total, transversal e nas partes centrais do calorímetro, dentre
...
obtém 72,4\% de eficiência na deteção de elétrons contra 39\% de falso-alarme
...
simples e normalizadas de acordo a parâmetros do objeto que se deseja
discriminar.
...
sejam selecionados por um corte do LVL1 devem ser simulados, uma vez que o LHC
ainda não encontra-se operacional. Desta forma, os termos \textit{conjunto de
teste} e \textit{conjunto de validação} serão utilizados de forma
intercambiável no contexto deste trabalho.
...
Figura~\ref{fig:lms-vs-egamma-roc} mostra um comparativo entre o resultado da
...
recuperação da informação mesmo em regiões mais dificultadas, como no
\eng{crack}.

\begin{figure}
\begin{center}
\mbox{%
 \subfigure[Eficiência por $\eta$]{\includegraphics[scale=0.49]{lms/test-efficiency-eta}}%
 \subfigure[Eficiência por $\phi$]{\includegraphics[scale=0.49]{lms/test-efficiency-phi}}%
}
\mbox{%
 \subfigure[Produto SP por $\eta$]{\includegraphics[scale=0.49]{lms/test-sp-eta}}%
 \subfigure[Produto SP por $\phi$]{\includegraphics[scale=0.49]{lms/test-sp-phi}}%
}
\end{center}
...
\begin{figure}
\begin{center}
\subfigure[Eficiência]{\includegraphics[scale=0.6]{lms/test-efficiency-emet}}
\subfigure[Produto SP]{\includegraphics[scale=0.6]{lms/test-sp-emet}}
\end{center}
...
de deteção mostram-se pouco robustos a mudanças de condição como as que serão
...
utilizando-se do pacote \eng{NeuralRinger} e o fluxo de treinamento foi
...
Uma vez que estejamos utilizando o algoritmo de retropropagação resiliente dos
...
utilizada: realiza-se 5 testes com um número fixo de neurônios na camada
...
o produto SP encontram-se em faixas de valores muito próximas, sendo difícil sua
...
EMQ. No que tange a capacidade discriminante, aferida através do produto SP,
...
discriminativa. Na Figura~\ref{fig:best-t2calo-test-roc} vemos um comparativo
...
\begin{center}
\mbox{%
 \subfigure[Eficiência por $\eta$]{\includegraphics[scale=0.49]{t2calo-mlp/test-efficiency-eta}}%
 \subfigure[Eficiência por $\phi$]{\includegraphics[scale=0.49]{t2calo-mlp/test-efficiency-phi}}%
}
\mbox{%
 \subfigure[Produto SP por $\eta$]{\includegraphics[scale=0.49]{t2calo-mlp/test-sp-eta}}%
 \subfigure[Produto SP por $\phi$]{\includegraphics[scale=0.49]{t2calo-mlp/test-sp-phi}}%
}
\end{center}
\caption{Análise da eficiência de classificação e do produto SP para o
...
\begin{figure}
\begin{center}
\subfigure[Eficiência]{\includegraphics[scale=0.6]{t2calo-mlp/test-efficiency-emet}}
\subfigure[Produto SP]{\includegraphics[scale=0.6]{t2calo-mlp/test-sp-emet}}
\end{center}
\caption{Análise da eficiência de deteção de elétrons e falso-alarme em jatos
...
tange à recuperação de informação, mesmo em regiões onde exista pouca
informação disponível ou onde haja descontinuidades no sistema de deteção.

\begin{figure}
\begin{center}
\mbox{%
 \subfigure[Eficiência por $\eta$]{\includegraphics[scale=0.49]{t2calo-mlp-all/test-efficiency-eta}}%
 \subfigure[Eficiência por $\phi$]{\includegraphics[scale=0.49]{t2calo-mlp-all/test-efficiency-phi}}%
}
\mbox{%
 \subfigure[Produto SP por $\eta$]{\includegraphics[scale=0.49]{t2calo-mlp-all/test-sp-eta}}%
 \subfigure[Produto SP por $\phi$]{\includegraphics[scale=0.49]{t2calo-mlp-all/test-sp-phi}}%
}
\end{center}
...
e 120~GeV. Este resultado indica que, apesar da carência de dados na região,
...
EGammaHypo. O que escapa a este sistema de deteção é que exista informação
...
finalmente chegando a zero, esperançosamente, em algum ponto antes do último
índice.
...
Treina-se 5 redes neurais (com inicializações diferentes) para cada uma das 6
...
neural parece elevar o poder de discriminação do LVL2, indicando que existam
...
células que se compõe uma RoI, e se tentar recuperar mais informações sobre a
...
é menos afetada do que desejaríamos. Isto se deve, principalmente, à agressiva
...
um conjunto de razões cujo o denominador diminui, de forma relacionada à
...
Baseando-se no número de neurônios destacado anteriormente, treinou-se dez
...
não existe mais interação da partícula com o detetor, e portanto a informação
...
valor de energia do anel $N$. Portanto, para manter o anel $N+1$ de uma
camada, é necessário que se mantenha o anel $N$, ao menos para o cálculo do
fator de normalização. De fato, como veremos mais a frente, esta última
...
este efeito de forma bastante evidente. Não mais se vê níveis de relevância de
discriminação negativos, o que indica que estejamos no limite do que é
